    ...
    def run(self, batches: list[list[git.Commit]]) -> None:
        with tqdm(total=len(batches), desc="Energy Pipeline", unit="batch") as progress_bar:
            for batch in batches:
                ...
                unique_commits = set(batch)
                with tqdm(
                    total=len(unique_commits), desc="Pre batch stages", unit="commit", leave=False
                    ...
                    for commit in unique_commits:
                        ...
                        if not self._run_stage_group(self.stages.get("pre_test_stages", []), commit_context):
                            return
