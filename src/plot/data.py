...
class EnergyData:
    ...
    def filter_outliers(self, df: pd.DataFrame, column: str, multiplier: float = 1.5) -> pd.DataFrame:
    ...
    def compute_commit_statistics(self, df: pd.DataFrame, energy_column: str) -> EnergyStats:
    ...
    def get_change_direction(self, baseline_median: float, test_median: float) -> str | None:
    ...
    def detect_energy_changes(self, distributions: list[np.ndarray]) -> list[ChangeEvent]:
        ...
        for i in range(1, len(distributions)):
            ...
            if p_value < WELCH_P_THRESHOLD:
                direction = self.get_change_direction(baseline_median, test_median)
                if direction:
    ...
    def process_column(self, energy_column: str) -> None:
        ...
        df_filtered = self.filter_outliers(self.df, energy_column)
        stats = self.compute_commit_statistics(df_filtered, energy_column)
        distribution, normality = self.compute_distribution_and_normality(df_filtered, stats.valid_commits, energy_column)
    ...
    def compute_overall_summary(self, energy_column: str) -> dict[str, Any]:
        ...
        outliers_removed_count = len(self.df) - len(self.filter_outliers(self.df, energy_column))
        summary = {
