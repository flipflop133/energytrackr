...
class LoadCSV(Transform):
    ...
    def __init__(self, csv_columns: list[str] | None = None) -> None:
        """Initialize the LoadCSV transform.

        Args:
            csv_columns (list[str] | None): Optional list of column names to use for the DataFrame.
                If None, defaults to settings.energytrackr.data.csv_columns.
        """
    ...
    def apply(self, ctx: Context) -> None:
        ...
        if (columns := self.csv_columns) is None:
